...
{
    private $dis = [
        'select', 'insert', 'delete', 'update', 'drop table', 'union', 'null', 'order by', 'order+by', 'from',
        'version', 'database', 'tables', 'query', '<', '>', '<script', '/>'
    ];
    ...
    public function validation($input = [])
    {
        ...
        foreach ($input as $key => $val) {
            ...
            if (in_array($key, $config['email'])) {
                $this->ValidateEmail($formData[$key]);
            }//only online
    ...
    private function ValidateText($text, $minlength, $maxlength, $allowed, $required)
    {
        ...
        if ($result) {
            $error[] = "The following character$plural not allowed: " . $caught;
        }
