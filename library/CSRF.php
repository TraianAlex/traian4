 ...
 */
    ...
    public static function checkToken($post_v)
    {
        ...
        if(!Sessions::exist('n') ||
            ...
            empty($post_v) ||
            $tok !== Sessions::get('n') ||
            $post_v !== Sessions::get('v') ||
            !Sessions::exist('time') ||
            (time() - Sessions::get('time') >= 180)
        ) {
 ...
 */
    ...
    {
        if(!Sessions::exist('token') ||
            !Input::get('token') ||
            Input::exist('token', 'get')=== false ||
            Input::get('token') !== Sessions::get('token') ||
            !Sessions::exist('time') ||
            (time() - Sessions::get('time') >= 120)
        ) {
            Errors::handle_error2(null,'&#x2718; Try again.');
        }
    }
